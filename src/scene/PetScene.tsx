        ...
        {[-2, -1.45, 1.55, 2.1].map((x, index) => (
          <group key={x} position={[x, 0, index % 2 * 0.2]} scale={0.7 + index * 0.08}>
            <mesh castShadow>
              ...
              <meshStandardMaterial color="#f2dfb5" />
            </mesh>
            <mesh castShadow position={[0, 0.3, 0]}>
              <sphereGeometry args={[0.28, 16, 10, 0, Math.PI * 2, 0, Math.PI / 2]} />
              <meshStandardMaterial color={index % 2 ? '#d6755f' : theme.accent} />
            </mesh>
          </group>
        ))}
      </group>
    ...
    return (
      <group position={[1.9, 1.1, -1.4]}>
        <mesh>
          ...
          <meshStandardMaterial color={theme.glow} emissive={theme.glow} emissiveIntensity={0.35} roughness={0.9} />
        </mesh>
  ...
  return (
    <group position={[-1.9, -0.5, -1.2]}>
      <mesh castShadow position={[0, 0.55, 0]}>
        ...
        <meshStandardMaterial color="#6e4d3c" />
      </mesh>
      {[[0, 1.35], [-0.45, 1.05], [0.42, 1.05], [-0.22, 1.62], [0.28, 1.58]].map(([x, y], index) => (
        <mesh castShadow key={index} position={[x, y, 0]}>
          <sphereGeometry args={[0.37, 14, 10]} />
          <meshStandardMaterial color={index % 2 ? '#ff9ca4' : '#ffc5bd'} />
        </mesh>
      ))}
    </group>
  )
